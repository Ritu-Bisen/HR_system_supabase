...
import { enquiryDataForProcessing, followUpDataSlice, followUpForHistory } from '../redux/slice/callTrackerSlice';
...
//  const [error, setError] = useState(null);
  ...
  const {enquiryForCall,loading,error,calltracker}=useSelector((state)=>state.callTracker);
  const dispatch=useDispatch();
  useEffect(()=>{
    dispatch(enquiryDataForProcessing())
    dispatch(followUpForHistory())
...
// Fetch initial data
  ...
  const fetchData = async () => {
    try {
      await dispatch(enquiryDataForProcessing());
      await dispatch(followUpForHistory());
    } catch (error) {
      console.error("Error fetching data:", error);
      toast.error("Failed to fetch data");
    }
...
const formatDOB = (dateString) => {
 ...
 const handleSubmit = async (e) => {
    ...
    try {
      const formattedNextDate = formData.nextDate
        ? new Date(formData.nextDate).toISOString().split("T")[0]
        : null;

      const rowData = {
        enquiry_no: selectedItem.candidate_enquiry_no || "",
        status: formData.status,
        candidate_says: formData.candidateSays,
        next_date: formattedNextDate || null,
      };

      // Submit the follow-up data
      const result = await dispatch(followUpDataSlice(rowData));
      
      if (result.error) {
        throw new Error(result.error.message || "Failed to update follow-up");
      }
  ...
  const filteredPendingData = enquiryForCall.filter(item => {
  ...
  const filteredHistoryData = calltracker.filter(item => {
  const matchesSearch = item.enquiry_no?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                     item.candidate_says?.toLowerCase().includes(searchTerm.toLowerCase());
  return matchesSearch;
});
                  ...
                  {loading ? (
                    ...
                    filteredHistoryData.map((item, index) => (
                      <tr key={index} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          {formatDOB(item.next_date) || "-"}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          {formatDOB(item.timestamp) || "-"}
                        </td>
                      </tr>
                    ))
                  )}
          ...
          <div className="bg-white rounded-lg shadow-lg w-full max-w-md">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                ...
                <input
                  type="text"
                  value={selectedItem.candidate_enquiry_no}
                  disabled
